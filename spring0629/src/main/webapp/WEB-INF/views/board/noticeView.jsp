						...
						<div id="mypage">
							...
							<div class="viewDivMt">
								<div class="viewHead">
									...
									<div class="day">
										<p class="txt">
											작성자<span>${bdto.id}</span>
										</p>
										<p class="txt">
											작성일<span> <fmt:formatDate value="${bdto.bdate}"
													pattern="yyyy-MM-dd" />
											</span>
										</p>
										<p class="txt">
											조회수<span>${bdto.bhit}</span>
										</p>
									</div>
								</div>
							...
							<div class="replyBox">
								<c:forEach var="comDto" items="${comList }">
									<ul id="${comDto.cno}">
										...
										<c:if
											test="${!(sessionId != comDto.id && comDto.cpw !=null) }">
											<li class="txt">${comDto.ccontent }</li>
										</c:if>

										<%-- 자신의 댓글이 아닌경우 버튼노출 안됨 --%>
										<!-- sessionId와 conDto.id가 같을 때만 수정, 삭제 노출 -->
