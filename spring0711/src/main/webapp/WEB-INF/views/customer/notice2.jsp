				...
				<div id="customer">
					...
					<div class="orderDivMt">
						<table summary="NO, 제목, 등록일, 조회수 순으로 공지사항을 조회 하실수 있습니다." class="orderTable2" border="1" cellspacing="0">
							...
							<tbody>
								<c:forEach var="memCusDto" items="${list}">
									<tr>
										<td class="tnone">${memCusDto.customerDto.bno}</td>
										<td class="left"><a
											href="/customer/noticeView?bno=${memCusDto.customerDto.bno}"
											class="lightgray">${memCusDto.customerDto.btitle} </a></td>
										<td><fmt:formatDate value="${memCusDto.customerDto.bdate}" pattern="yyyy-MM-dd"/></td>
										<%-- <td>${cdto.bdate}</td> --%>
										<td>${memCusDto.customerDto.bhit}</td>
									</tr>
								</c:forEach>
