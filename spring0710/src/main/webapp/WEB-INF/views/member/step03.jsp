...
<script type="text/javascript">
   function memberBtn(){
	   //alert("입력한 데이터 길이 : "+ name;
	   
	   let nameCheck = /^[ㄱ-ㅎ가-힣]+$/;     //^ : 시작 $ : 끝까지 + : 1개 이상 []:안에 있는 부분을 체크해줭  
	   let numCheck = /^[0-9]+$/;          //^시작부터 $끝까지 []안에 있는건지 +1개이상 봐줘
	   let alphaCheck = /^[a-zA-Z]+$/;     //영문자인지
	   let etcCheck = /^[!@#$%^&*]+$/;     //특수문자가 존재하는지
	   let idCheck=/^[a-zA-Z0-9_]{2,5}$/;    //{2,5}2자리에서 5자리까지 가능
	   let idCheck2 = /^[a-zA-Z]{1}[a-zA-Z0-9_]{2,5}$/;    //첫번째는 무조건 영문자만 가능
	   let pwCheck = /^(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[!@#$%^&*]).{2,7}$/ ;      //?:비교해라   //하나 들고와서 비교하고 또 하나 들고와서 비교해라
	   let phoneCheck = /^[0-9]{2,3}-[0-9]{3,4}-{0-9]{4}$/;    //010-1111-1111, 02-355-1111둘 다 가능
			   
			   
	   /* 한글만 입력할 수 있도록 걸기 */
	   /* if(!nameCheck.test($("#mem_name").val())){    //정규표현식 패턴과 입력한 글자 비교
		   alert("한글만 입력이 가능합니다.");
		   $("#mem_name").focus();      //커서 넘기기
		   return false;
	   }else{
		   alert("성공!!!");
	   } */
	   ...
	   //id는 영문이랑 숫자만 가능하도록 설정하기---------------------------------
	   if(!idCheck.test($("#mem_id").val())){    //정규표현식을 따로 하나 짜서 가져오는게 빠르다!! 두개를 엮을 생각하지말자,,
		   alert("영어랑 숫자,특수문자(_)만 입력이 가능합니다.2자리 이상 5자리 이하 가능");
		   $("#mem_id").focus();      //커서 넘기기
		   return false;
	   }else{
		   alert("성공!!");
	   }
	   //pw는 영문, 숫자, 특수문자가 1개이상인지 검색---------------------------------
	   if(!pwCheck.test($("#mem_pw").val())){    //정규표현식을 따로 하나 짜서 가져오는게 빠르다!! 두개를 엮을 생각하지말자,,
		   alert("영문, 숫자, 특수문자가 1개이상 입력하셔야 합니다");
		   $("#mem_pw").focus();      //커서 넘기기
		   return false;
	   }else{
		   alert("성공!!");
	   }
	   //pw2는 영문, 숫자, 특수문자가 1개이상인지 검색---------------------------------
	   if(!pwCheck.test($("#mem_pw").val())){    //정규표현식을 따로 하나 짜서 가져오는게 빠르다!! 두개를 엮을 생각하지말자,,
		   alert("영문, 숫자, 특수문자가 1개이상 입력하셔야 합니다");
		   $("#mem_pw").focus();      //커서 넘기기
		   return false;
	   ...
	   if(name_len < 2){
		   ...
		   $("#mem_name").focus();   //커서를 그 부분으로 다시 넘어가기
		   return false;
	   } */
						...
						<table summary="이름, 아이디, 비밀번호, 비밀번호 확인, 이메일, 이메일수신여부, 주소, 휴대폰, 유선전화, 생년월일 순으로 회원가입 정보를 등록할수 있습니다." class="memberWrite" border="1" cellspacing="0">
							...
							<tbody>
								...
								<tr>
									...
									<td>
										<ul class="pta">
											<li class="r10"><input type="text" onKeyUp = "pwKey()" class="w134" id="mem_pw2"/></li>
											<li id = "pwKeyText">
												<!-- 
											...
											<script>
											   function email_chn(){
												   ...
												   if($("#emailList").val()=="txt"){
													   $("#email2").val("");   //글자삭제
													   $("#email2").attr("readonly",false);  //disabled로 하면 form 에서 안넘어간다 : readonly로 해야함
													   $("#email2").focus();
												   }else{
													   $("#email2").val($("#emailList").val());
												       $("#email2").attr("readonly",true);  //속성값을 변경하는것!
												   }
